...
def _build_single_point(labels, texts, text_index, sentence_ids, sentence_texts):
    return [{
        "id": 0,
        "x": 0.0,
        "y": 0.0,
...
def main(embeddings_file="entity_embeddings.npz", output="web/tsne_data.json"):
    ...
    if len(embeddings) < 2:
        print("[WARN] No hay suficientes entidades para t-SNE. Exportando fallback directo.")
        points = _build_single_point(labels, texts, text_index, sentence_ids, sentence_texts) if len(embeddings) == 1 else []
    ...
    print("[STEP 3] Exportando JSON para visualizacion")
    points = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generar proyeccion t-SNE de embeddings de entidades SciBERT"
    )
    parser.add_argument(
        "--embeddings",
